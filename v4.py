...
for row, (date, time, message, script_type, operator, font) in enumerate(script_logs_data, start=2):
    ...
    for col, font_style in enumerate([font, font, font, font, font_default], start=1):
...
for row, (date, time, operator, status, font) in enumerate(all_logs_data, start=2):
    ...
    for col, font_style in enumerate([font, font, font_default, font], start=1):
...
output_file = f"V4_saida_logs_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.xlsx"
wb.save(output_file)
